...
사용법:
    ...
    result = await service.analyze(image_bytes, "image/jpeg")
"""
...
from .prompts import CHECKUP_PROMPT
from .schemas import CheckupResult
...
class CheckupService:
    ...
    async def analyze(
        ...
        """
        건강검진 결과지 이미지 분석
        - 모델: gpt-4o-mini | detail: high | temperature: 0
        - 결과: 출생연도, 성별, 신장, 체중, 혈압, 혈당, 콜레스테롤
        ...
        Returns:
            dict: {"result": CheckupResult dict, "usage": UsageInfo dict}
        """
        ...
        # 1순위: Pydantic 검증 — 타입 오류 / 필드 누락 조기 차단
        try:
    ...
    def _sanitize(self, r: CheckupResult) -> CheckupResult:
        """비현실적 수치를 null로 보정하고 필요시 confidence를 낮춤"""
        issues = []
