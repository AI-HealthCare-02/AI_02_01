...
async def migrate_guest_analysis(
    ...
    logger.info("[user_id=%d] 게스트 분석 이전 - guest_task_id=%s, record_id=%d", user.id, data.guest_task_id, data.record_id)
    service = HealthAnalysisService(session, redis)
    ...
    if current_user is not None:
        ...
        if raw_meta:
            ...
            if task_user_id is not None and task_user_id != current_user.id:
                logger.warning("[user_id=%d] 불법 task 조회 시도 - task_id=%s (owner=%s)", current_user.id, task_id, task_user_id)
                raise HTTPException(
...
async def wait_analysis_result(
    ...
    if current_user is not None:
        ...
        if raw_meta:
            ...
            task_user_id = meta.get("user_id")
            if task_user_id is not None and task_user_id != current_user.id:
                logger.warning("[user_id=%d] 불법 task wait 시도 - task_id=%s (owner=%s)", current_user.id, task_id, task_user_id)
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="본인의 분석 결과만 조회할 수 있습니다.",
                )
