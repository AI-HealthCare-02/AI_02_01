...
class User(Base):
    ...
    provider: Mapped[str] = mapped_column(
        String(20), nullable=False, default="google", comment="소셜 로그인 제공자 (현재 google만 지원)"
    )
    provider_id: Mapped[str] = mapped_column(
        String(128), unique=True, nullable=False, comment="소셜 로그인 제공자 고유 ID"
    )
    ...
    gender: Mapped[str | None] = mapped_column(
        String(1), nullable=True, comment="M: 남성 / F: 여성 (최초 입력 후 변경 불가)"
    )
    birth_year: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="출생 연도 (age 동적 계산 기준)")
    ...
    withdraw_reason: Mapped[str | None] = mapped_column(
        String(500), nullable=True, comment="탈퇴 사유 (재가입 시 초기화)"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, nullable=False, server_default=func.now(), comment="계정 생성 시각"
    )

    @hybrid_property
    def age(self) -> int | None:
        """birth_year 기반 나이 동적 계산 (DB 컬럼 없이 요청 시마다 재계산)"""
        if self.birth_year is None:
            return None
        return datetime.now().year - self.birth_year + 1
